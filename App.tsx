...
const App: React.FC = () => {
  ...
  const getViewTitle = () => {
      switch(currentView) {
          case AppView.DASHBOARD: return 'Home';
          case AppView.METADATA: return 'Item Search';
          case AppView.SCRAPING: return 'Deep Search';
          case AppView.ANALYTICS: return 'Analytics';
          case AppView.WAYBACK: return 'Wayback Machine';
          case AppView.SETTINGS: return 'Settings';
          default: return 'OmniDash';
      }
  };
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-gray-100 font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-900">
        {/* Top Header Bar */}
        <div className="h-16 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-8 shadow-sm">
            <h2 className="text-lg font-bold text-white tracking-wide">
                {getViewTitle()}
            </h2>
            <div className="flex items-center gap-4">
                 {settings.demoMode && (
                    <div className="px-3 py-1 rounded-md text-xs font-bold border border-yellow-500/30 text-yellow-400 bg-yellow-500/10">
                        DEMO MODE
                    </div>
                 )}
                 <div className="px-3 py-1 rounded-md text-xs font-bold border border-teal-500/30 text-teal-400 bg-teal-500/5">
                    API V1
                 </div>
                 {settings.accessKey && (
                     <div className="flex items-center gap-2 px-3 py-1 rounded-md text-xs font-medium bg-gray-800 text-gray-300 border border-gray-700">
