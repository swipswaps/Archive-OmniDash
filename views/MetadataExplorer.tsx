...
import { Search, Save, Trash2, FileText, Code, Sparkles, Database, ArrowRight, AlertCircle, Info, Link as LinkIcon, ExternalLink } from 'lucide-react';
...
const MetadataExplorer: React.FC<Props> = ({ settings, onChangeView, onSearchTransfer }) => {
  ...
  const handleFetch = async (e?: React.FormEvent, idOverride?: string) => {
    ...
    if (targetId.includes('.') && !targetId.includes(' ') && !targetId.includes('_') && !targetId.includes('-')) {
    ...
    try {
      ...
      if (!result || Object.keys(result).length === 0 || (result.metadata && Object.keys(result.metadata).length === 0)) {
                ...
                {onSearchTransfer && (
                    <div className="ml-8 text-sm text-red-300/80 space-y-3">
                         {identifier.includes('archive.org') ? (
                            <p>It looks like you pasted a URL but we couldn't extract a valid ID. Try pasting just the identifier (the part after <code>/details/</code>).</p>
                         ) : isLikelySearchQuery(identifier) ? (
                            <div className="flex flex-col items-start gap-2">
                                <p>"{identifier}" looks like a search query or a generic URL, not an Item Identifier.</p>
                                <Button 
                                    variant="secondary" 
            ...
            <div className="flex space-x-6">
              <button
                ...
                className={`flex items-center gap-2 py-4 border-b-2 text-sm font-medium transition-all ${
                  activeTab === 'view' ? 'border-teal-500 text-teal-400' : 'border-transparent text-gray-400 hover:text-gray-200'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 py-4 border-b-2 text-sm font-medium transition-all ${
                  activeTab === 'json' ? 'border-teal-500 text-teal-400' : 'border-transparent text-gray-400 hover:text-gray-200'
                }`}
              >
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div>
                        <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Core Metadata</h3>
                        <div className="bg-gray-900/50 rounded-xl p-5 space-y-4 border border-gray-700/50">
                        <div>
                            <label className="text-xs text-gray-500 block mb-1">Title</label>
                            <p className="text-xl font-bold text-white">{data.metadata?.title || 'Untitled'}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs text-gray-500 block mb-1">Identifier</label>
                                <p className="text-gray-300 font-mono text-sm bg-gray-950 px-2 py-1 rounded inline-block select-all">{data.metadata?.identifier}</p>
                            </div>
                             <div>
                  ...
                  <div className="space-y-6">
                    <div>
                        <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Statistics</h3>
                        <div className="grid grid-cols-2 gap-4">
                        <div className="bg-gray-900/50 rounded-xl p-5 border border-gray-700/50">
                            <div className="text-3xl font-bold text-teal-400 mb-1">{data.files_count}</div>
                            <div className="text-xs text-gray-500 font-medium uppercase">Total Files</div>
                        </div>
                        <div className="bg-gray-900/50 rounded-xl p-5 border border-gray-700/50">
                            <div className="text-3xl font-bold text-indigo-400 mb-1">{((data.item_size || 0) / 1024 / 1024).toFixed(2)} MB</div>
                            <div className="text-xs text-gray-500 font-medium uppercase">Total Size</div>
                        </div>
                        </div>
                    </div>
                  </div>
                ...
                <div>
                   <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">File Listing</h3>
                       ...
                       <tbody className="divide-y divide-gray-700/50">
                         {data.files?.slice(0, 15).map((file, idx) => (
                           <tr key={idx} className="hover:bg-gray-800 transition-colors">
                             <td className="px-6 py-3 text-gray-300 font-mono truncate max-w-xs" title={file.name}>{file.name}</td>
                             <td className="px-6 py-3 text-gray-400">{file.format}</td>
                             <td className="px-6 py-3 text-gray-400">{file.size}</td>
                             <td className="px-6 py-3 text-gray-400">
                               <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${file.source === 'original' ? 'bg-teal-500/10 text-teal-400 border border-teal-500/20' : 'bg-gray-700 text-gray-400'}`}>
                                 {file.source}
                               </span>
                             </td>
                           </tr>
                         ...
                         {(data.files?.length || 0) > 15 && (
                            <tr>
                              <td colSpan={4} className="px-6 py-3 text-center text-gray-500 text-xs italic bg-gray-900/30">
                    ...
                    <div className="flex items-start gap-6">
                        ...
                        <div className="space-y-6 flex-1">
                            ...
                            <div className="bg-gray-900 rounded-lg p-5 border border-gray-700 shadow-inner">
                                <div className="text-gray-500 mb-3 text-xs uppercase tracking-wider font-bold">Supported Inputs</div>
                                <div className="space-y-3">
                                    <div>
                                        <div className="text-xs text-gray-500 mb-1">1. Archive.org URL (We auto-extract the ID)</div>
                                        <div className="font-mono text-sm text-gray-300 bg-gray-950 p-2 rounded border border-gray-800">
                                            https://archive.org/details/<span className="text-teal-400 font-bold">nasa_audio_collection</span>
                                        </div>
                                    </div>
                                    <div>
